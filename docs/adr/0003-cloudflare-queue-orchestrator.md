...
We use a Cloudflare Queue (`queue()` handler) as the orchestrator. The `fetch()` handler receives Linear webhooks and enqueues a job in ~100ms (well under the 5-second webhook-ack limit); the queue consumer owns the full job lifetime with unlimited CPU time and wall time measured in minutes. The consumer calls `POST /session/:id/prompt_async` (fire-and-forget) on the opencode server, then polls `GET /session/:id/message` every 5 seconds for new Parts, translating each to a Linear AgentActivity.
