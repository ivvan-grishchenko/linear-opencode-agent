...
The directory \`openspec/changes/${change.name}/\` should also exist in the repo
as the canonical home for this change; do not modify its contents.

## Workflow
1. Read the spec in the issue description above to understand the intended change.
2. Verify that \`openspec/changes/${change.name}/\` exists in the repo.
   If it does not exist, stop and report: "OpenSpec change directory
   openspec/changes/${change.name}/ not found. Run \`openspec create ${change.name}\`
   locally and push, then re-assign this issue."
3. Create a branch: \`git checkout -b ${change.branchName}\`
   If the branch already exists, check it out with \`git checkout ${change.branchName}\`.
4. Implement the change following the spec.
5. Run tests / typecheck / lint as appropriate for this repo.
6. Commit your changes with a clear message referencing ${identifier}.
7. Push and open a PR:
   \`gh pr create --title "feat: ${change.name}" --body "Implements ${change.name}. Closes ${identifier}."\`
8. Report the PR URL in your final response.
